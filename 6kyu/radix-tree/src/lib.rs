...
unsafe fn radix_tree_(mut words: &mut [&str], ignored_bytes: usize) -> RadixTree {
    ...
    while let Some(base_word) = words.iter().next() {
        let base_word = base_word.get_unchecked(ignored_bytes..);
        ...
        for i in 1..words.len() {
            let second_word = words[i].get_unchecked(ignored_bytes..);
            if !second_word.starts_with(first_ch) {
                continue;
            }

            min_cmn_pref = min_cmn_pref.min(second_word.len());
            for (i, (b1, b2)) in base_word
                .get_unchecked(..min_cmn_pref)
                .bytes()
            ...
            words.swap_unchecked(i, group_len);
            group_len += 1;
        }
