...
fn bench_ascii(bencher: &mut Bencher) {
    ...
    const TEXT_MIRI: &str = "Many recent philosophers";

    bencher.iter(|| encrypt_this(black_box(if cfg!(miri) { TEXT_MIRI } else { TEXT })));
}
...
fn bench_nonascii(bencher: &mut Bencher) {
    ...
    const TEXT_MIRI: &str = "Дионисий Младший снова";
