...
extern crate test;
use makeuppercase::make_upper_case;
...
fn bench_ascii(bencher: &mut Bencher) {
    ...
    const S_MIRI: &str = "Many recent philosophers have also diverged";

    bencher.iter(|| make_upper_case(black_box(if cfg!(miri) { S_MIRI } else { S })));
}
...
fn bench_nonascii(bencher: &mut Bencher) {
    ...
    const S_MIRI: &str = "Дионисий Младший снова призывает Платона";
