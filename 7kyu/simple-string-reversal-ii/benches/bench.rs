...
extern crate test;
use simple_string_reversal_ii::solve;
...
fn bench_ascii(bencher: &mut Bencher) {
    ...
    const S_MIRI: &str = "Many recent philosophers have also";

    let s = if cfg!(miri) { S_MIRI } else { S };
    bencher.iter(|| {
        solve(
            black_box(s),
            black_box(s.len() / 3 + 1),
            black_box(s.len() * 2 / 3 + 2),
        )
    });
}
...
fn bench_nonascii(bencher: &mut Bencher) {
    ...
    const S_MIRI: &str = "Дионисий Младший снова призывает";
